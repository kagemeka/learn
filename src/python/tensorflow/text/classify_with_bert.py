...
dataset = tf.keras.utils.get_file(
  fname=f'{root}/aclImdb_v1.tar.gz',
  origin=url,
  untar=True, 
  cache_dir=root,
  cache_subdir='',
...
train_ds = raw_train_ds.cache().prefetch(
...
print(
  f'Keys : {list(text_preprocessed.keys())}')
print(
  f'Shap: {text_preprocessed["input_word_ids"].shape}')
print(
  f'word ids: {text_preprocessed["input_word_ids"][0, :12]}'
)
...
def build_classifier_model():
  ...
  encoder = hub.KerasLayer(
    tfhub_handle_encoder, trainable=True,
    name='BERT_encoder',
  )
...
epochs = 5
steps_per_epoch = tf.data.experimental \
  .cardinality(train_ds).numpy()
...
plt.subplot(2, 1, 2)
plt.plot(
  epochs, acc, 'r', label='training acc')
plt.plot(
  epochs, val_acc, 
...
reloaded_results = tf.sigmoid(
  reloaded_model(tf.constant(examples))
)
original_results = tf.sigmoid(
  classifier_model(tf.constant(examples))
)
...
serving_results = tf.sigmoid(
  serving_results['classifier'])
